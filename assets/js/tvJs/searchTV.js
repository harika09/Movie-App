...
async function getTV() {
  ...
  const response = await fetch(
    `https://api.themoviedb.org/3/search/tv?api_key=${API_key}&language=en-US&page=1&query=${tvTitle}&include_adult=false`
  );
...
function showTrailer(key) {
  ...
  btnTrailer.addEventListener("click", function () {
    if (trailerContainer.style.display !== "none") {
      ...
      trailer.innerHTML = `
          <iframe class="video-trailer" src="${
            youtubeURL + keys
...
function showTv(tv) {
  tv.forEach((tvData) => {
    ...
    tvEL.innerHTML = `<img src="${
      image_path + poster_path
    }" alt=${original_name} id="poster" onerror="this.src = '/assets/img/poster-placeholder.svg'">
        ...
        <div>
            ...
            <div class="movie-release-container">
                <div class="movie-date">
                        <p>Release Date: ${movie.first_air_date
                          .replace("-", "/")
                          .replace("-", "/")}</p>
                </div>
...
function getTVRecomData(recomData) {
  ...
  recomData.forEach((recomDatas) => {
    ...
    recommendation_list.innerHTML = `
            ...
            <img class="recommendation-img" src="${
              image_path + poster_path
            }" onerror="this.src = '/assets/img/poster-placeholder.svg'">
